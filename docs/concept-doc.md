...
- **Next Pick:** The pure best-player-available pick, independent of the active strategy, shown as its own card (position badge, team/bye, key stats — points, value, ADP) with its reasoning spelled out, plus a short "then" preview of the next few names as compact cards — a quick "if I ignore roster need entirely" cross-check.
- **Strategy:** Names the active strategy, shows its single top suggestion (roster/round-aware) as a card with the same key stats and plain-language reasoning. Nests **Also Consider** underneath as a compact card row — what each of the other three named strategies would suggest right now, replacing the originally-envisioned live BPA/Need slider.
...
Each Draft Board section (My Team, Your Draft, Next Pick, Strategy, Available Players) has its own light background tint so the panels read as distinct zones without needing to read every heading. Drafts can also be saved under a user-chosen label (independent of the always-on autosave) and resumed later from the League Setup screen.
